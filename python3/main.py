...
def introduce(player):
    if player["winPercent"]:
        print("{:s} wins {:f}% of the time.".format(player["name"], player["winPercent"]))
    else:
        print("{:s} is a new player.".format(player["name"]))

playerSchema = {
    "type": "object",
    "required": ["name", "winPercent"], # maybe "winPercent" too?
    "properties": {
        "name": {"type": "string"},
        "winPercent": {"type": ["number", "null"]}, # does not support float
    },
}
